...
export const updateProfile = user => async (
  ...
  try {
    //update the profile inside firestore
    await firebase.updateProfile(updatedUser);
    //update the profile inside firebase authentication
    if (authUser.displayName !== updatedUser.displayName) {
...
export const uploadProfileImage = (file, fileName) => async (
  ...
  try {
    ...
    const firestoreUserDoc = await firestore.get(`users/${user.uid}`);
    //check if user has photo, if not update profile with new image
    if (!firestoreUserDoc.data().photoURL) {
